 ...
 * payment immediately (idempotently) so the portal reflects it right away, even
 * if the webhook is delayed or not configured; the webhook is a redundant
...
export async function GET(
  ...
  if (!ro) return NextResponse.redirect(portal, { status: 303 });
