              ...
              <div className="divide-y divide-zinc-200">
                {review.topSellingProducts.map((product) => (
                  <div key={product.itemName} className="flex items-center justify-between px-5 py-3 text-sm">
                    <span className="text-zinc-700">{product.itemName} · {product.units} sold</span>
                    <span className="font-medium tabular-nums text-zinc-900">{formatMoney(product.revenue)}</span>
                  </div>
                ))}
              </div>
            )}
              ...
              {review.activeGoals.map(({ goal, progress }) => (
                <div key={goal.id} className="flex items-center justify-between gap-3 px-5 py-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-zinc-500">{goalMetricLabel(goal.metric, user.accountType, features.has("invoices"))} · {goalValueLabel(goal.metric, progress.actual)} of {goalValueLabel(goal.metric, progress.target)}</p>
                  </div>
